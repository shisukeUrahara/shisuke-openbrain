...
- **Never attribute commits or PRs to Claude in any form.** No `Co-Authored-By: Claude` trailers (with or without a model suffix), no `🤖 Generated with Claude` footers, no mention of Claude, Anthropic, or any model name (`claude-opus-4-7`, `Claude Opus 4.7`, `Sonnet`, `Haiku`, etc.) anywhere in commit messages, PR titles, or PR bodies. Commits stand on their content alone.
